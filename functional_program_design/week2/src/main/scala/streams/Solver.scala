...
trait Solver extends GameDef {
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    if (initial.isEmpty) Stream.empty
      ...
      val nextIter = initial.flatMap({ case (block, moves) => newNeighborsOnly(neighborsWithHistory(block, moves), explored) })
      initial ++ from(nextIter, explored ++ nextIter.map({ case (block, _) => block}))
    }
